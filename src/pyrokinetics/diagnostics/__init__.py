...
class Diagnostics:
    ...
    def flux_surface_average_density(self, ntheta_multiplier: int = 4):
        r"""
        Calculates the flux surface average of the density of each species,
        including the poloidal variation generated by sonic rotation.

        The density of each species varies poloidally as

        .. math::
            n_s(\theta) = n_s(\theta=0)
            \exp\left[\chi_s(\theta=0) - \chi_s(\theta)\right],
            \quad
            \chi_s = \frac{Z_s\phi_0}{T_s}
            - \frac{m_s\Omega_0^2R^2}{2T_s}

        where :math:`\phi_0` is the potential satisfying quasineutrality
        obtained from
        :func:`~Diagnostics.calculate_sonic_quasineutrality` and
        :math:`n_s(\theta=0)` is the density stored in ``local_species``,
        defined at the outboard midplane.

        The flux surface average of a quantity :math:`A` is

        .. math::
            \langle A\rangle = \frac{\oint A\mathcal{J}\,d\theta}
            {\oint\mathcal{J}\,d\theta}

        with the Jacobian of the flux surface

        .. math::
            \mathcal{J} = R\left(\frac{\partial R}{\partial r}
        ...
        ntheta_multiplier: int
            Multiplier of numerics.ntheta setting the resolution of the
            poloidal grid used in the quasineutrality solve and integrals

        Returns
        -------
        flux_surface_average_density: dict
            Dictionary of the flux surface averaged density of each species
        ...
        # Jacobian of the flux surface, as defined in MetricTerms
        jacob = R * (dR_dr * dZ_dtheta - dZ_dr * dR_dtheta)
        jacob_integral = simpson(jacob.m, x=theta) * jacob.units

        omega0 = local_species.electron.omega0

        R_theta_zero = R[theta_zero]
        phi0_theta_zero = phi0[theta_zero]

        flux_surface_average_density = {}
        for name in local_species.names:
            species = local_species[name]
            z = species.z
            mass = species.mass
            ...
            potential0 = (
                z * phi0_theta_zero / temp
                - mass * omega0**2 * R_theta_zero**2 / (2 * temp)
            )
            potential = z * phi0 / temp - mass * omega0**2 * R**2 / (2 * temp)

            dens_theta = dens * np.exp(potential0 - potential)

            dens_integral = simpson((dens_theta * jacob).m, x=theta) * (
                dens_theta.units * jacob.units
            )

            flux_surface_average_density[name] = dens_integral / jacob_integral

        return flux_surface_average_density
