...
class SyntheticHighkDBS:
    ...
    def mapk(self):
        ...
        grtheta = self.metric.toroidal_contravariant_metric("r", "theta")  # gradr . gradtheta
        gthetatheta = self.metric.toroidal_contravariant_metric("theta", "theta")  # |grad theta|^2
        ...
        # gradients of alpha
        galpha_dot_gr = self.metric.dalpha_dr * grr + self.metric.dalpha_dtheta * grtheta
        galpha2 = ( self.metric.dalpha_dr**2 * grr
        ...
        dalpha_dr0 = np.interp(self.thetaloc, self.metric.regulartheta, self.metric.dalpha_dr)
        dalpha_dtheta0 = np.interp(self.thetaloc, self.metric.regulartheta, self.metric.dalpha_dtheta)
        ...
        dRdtheta0 = np.interp(self.thetaloc, self.metric.regulartheta, self.metric.dRdtheta)
        dZdr0 = np.interp(self.thetaloc, self.metric.regulartheta, self.metric.dZdr)
        dZdtheta0 = np.interp(self.thetaloc, self.metric.regulartheta, self.metric.dZdtheta)
        R0 = np.interp(self.thetaloc, self.metric.regulartheta, self.metric.R )
        gtheta_cross_gr20 = np.interp(self.thetaloc, self.metric.regulartheta, gtheta_cross_gr2)
        gphi_cross_gr20 = np.interp(self.thetaloc, self.metric.regulartheta, gphi_cross_gr2)

        galpha_dot_gr0 = np.interp(self.thetaloc, self.metric.regulartheta, galpha_dot_gr)
        galpha20 = np.interp(self.thetaloc, self.metric.regulartheta, galpha2)
        galpha_cross_gr_20 = np.interp(self.thetaloc, self.metric.regulartheta, galpha_cross_gr_2)
        bcross_gradr_dot_gradalpha0 = np.interp(self.thetaloc, self.metric.regulartheta, bcross_gradr_dot_gradalpha
        )
        ...
        if self.filter_type == "gauss":
            ...
            self.dky0 = 2 * self.geometry.kappa * self.metric.q / (self.wZ * dalpha_dtheta0) * self.rhos_unit 
            # elif filter == 'beam':
    ...
    def get_syn_fspec(self, t1, t2, savedir, if_save):
        ...
        for ik in np.arange(np.size(self.Kn0_exp)):
            ...
            ## call filter
            self.filter = Filter(self.filter_type, self.kx, self.ky, self.kx0[ik], self.ky0[ik], self.dkx0, self.dky0 )
            
            ## filter fluct: scattered power
            self.ps_locsyn = self.apply_filter(phikxkyt, self.filter.F2, dims = ['kx', 'ky'])
            self.ps_hann = self.apply_filter( phikxkyt*np.hanning(np.size(self.sim_time)) , self.filter.F2, dims = ['kx', 'ky'])   # ps using hanning window in time
            self.ps_kxavg = self.apply_filter( phikxkyt, self.filter.F2_kxavg, dims = ['kx', 'ky'])
            self.ps_kxavg_nz = self.apply_filter( phikxkyt, self.filter.F2_kxavg_nz, dims = ['kx', 'ky'])
            self.ps_kxavg_zon = self.apply_filter( phikxkyt, self.filter.F2_kxavg_zon, dims = ['kx', 'ky'])
            self.ps_nz = self.apply_filter( phikxkyt, self.filter.F2_nz, dims = ['kx', 'ky'])
            self.ps_zon = self.apply_filter( phikxkyt, self.filter.F2_zon, dims = ['kx', 'ky'])
            self.dne2_locsyn_ky = self.apply_filter( phikxkyt, self.filter.F2, dims = ['kx', 'time'])
            self.dne2_kxavg_ky = self.apply_filter( phikxkyt, self.filter.F2_kxavg, dims = ['kx', 'time'])
            ...
            ps_re_dop = np.sum(np.sum(np.real(phikxkyt_dop) ** 2 * self.filter.F2, 0), 0)

            phikxkyf = xrft.fft( phikxkyt, true_phase=True, true_amplitude=True, dim=["time"] )  # Fourier Transform w/ consideration of phase
            phikxkyf_hann = xrft.fft( phikxkyt * np.hanning(np.size(self.sim_time)), 
                true_phase=True,
                true_amplitude=True,
                dim=["time"],
            )  # Fourier Transform w/ consideration of phase
            
            self.phi2f_f2_locsyn = self.apply_filter(phikxkyf, self.filter.F2, dims = ['kx', 'ky'])
            self.phi2f_f2_kxavg = self.apply_filter(phikxkyf, self.filter.F2_kxavg, dims = ['kx', 'ky'])  
            self.phi2f_f2_kxavg_nz = self.apply_filter(phikxkyf, self.filter.F2_kxavg_nz, dims = ['kx', 'ky']) 
            self.phi2f_f2_kxavg_zon = self.apply_filter(phikxkyf, self.filter.F2_kxavg_zon, dims = ['kx', 'ky'])    
            self.phi2f_f2_nz = self.apply_filter(phikxkyf, self.filter.F2_nz, dims = ['kx', 'ky'])
            self.phi2f_f2_zon = self.apply_filter(phikxkyf, self.filter.F2_zon, dims = ['kx', 'ky']) 
            self.phi2f_f2_hann = self.apply_filter(phikxkyf_hann, self.filter.F2, dims = ['kx', 'ky']) 
            ...
            phikxkyfdop = xrft.fft( phikxkyt_dop, true_phase=True, true_amplitude=True, dim=["time"] )  # Fourier Transform w/ consideration of phase
            phi2fdop_f2 = self.apply_filter( phikxkyfdop, self.filter.F2, dims = ['kx', 'ky'] )
            #phi2fdop_f2 = np.sum(np.sum(np.abs(phikxkyfdop) ** 2 * self.filter.F2, 0), 0)

            f0_avg = (self.phi2f_f2_locsyn.freq_time * self.phi2f_f2_locsyn).sum(dim="freq_time") / (
            ...
            deltaf_theory = self.ky0[ik] * w0 * self.geometry.rho / (2 * np.pi * self.geometry.q)  # [vt/a]
            ...
            plt.xlabel(r"$k_y\rho_s$", fontsize=self.fsize)
            plt.title( r"$|\delta \hat{n}|^2(k_y)_{k_{xDBS}}$, ch = " + str(ik),
                fontsize=self.fsize,
            )
            #plt.legend()
            plt.tick_params(labelsize=self.fsize)
            ...
            plt.plot(
                self.sim_time.data, pks.data * np.ones(np.size(self.ps_locsyn.data)), "--", c="b"
            )
            ...
            plt.subplot(1, 2, 1)
            plt.plot(self.ky, self.filter.Fy2_shift, ".-", c="b", lw=2, label="ch = " + str(ik) + " (shift)")
            plt.xlabel(r"$k_y\rho_s$", fontsize=self.fsize)
            plt.title( r"$|F_y(k_y)|^2$", fontsize=self.fsize)
            #plt.legend()
            plt.tick_params(labelsize=self.fsize)
