...
class PyroHypercube(PyroScan):
    ...
    **kwargs
    ...
    and write the same points out as TGLF input files, leaving the GS2 runs
    untouched::

        cube.convert_gk_code("TGLF")
        cube.write(base_directory="tglf")
    """
    ...
    def _discover_runs(
        ...
        if len({path.name for path in input_files}) > 1:
            other = next(p for p in input_files if p.name != input_files[0].name)
            raise ValueError(
                "PyroHypercube.from_directory: every run must use the same input "
                f"file name, but found '{input_files[0].name}' in "
                f"{input_files[0].parent} and '{other.name}' in {other.parent}. "
                "Pass file_name to choose one."
            )

        return names, input_files

    @staticmethod
    def _check_reference_values(names: Sequence[str], pyros: Sequence[Pyro]) -> None:
        """
        Warn if the runs do not share the first run's physical reference values.

        Output is stored and converted with one set of reference values, those
        of the first run, as for a ``PyroScan``. Runs read from disk each carry
        their own, and where these differ, every run but the first is
        converted with the wrong ones. Runs without physical reference values
        are all in the same simulation units, and so cannot disagree.

        ``lref_major_radius`` is not compared: it follows ``Rmaj``, which may be
        one of the varied parameters.
        """

        def references(pyro):
            try:
                values = pyro.get_reference_values()
            except Exception:
                return None
            values.pop("lref_major_radius", None)
            return values

        first = references(pyros[0])
        if first is None:
            return

        differing = {}
        for name, pyro in zip(names[1:], pyros[1:]):
            values = references(pyro)
            if values is None:
                differing.setdefault(name, []).append("all (no physical references)")
                continue
            for key, value in values.items():
                if not np.isclose(
                    value.to(first[key].units).magnitude, first[key].magnitude
                ):
                    differing.setdefault(name, []).append(key)

        if differing:
            example, keys = next(iter(differing.items()))
            warnings.warn(
                f"PyroHypercube.from_directory: {len(differing)} run(s) have "
                f"physical reference values different from the first run "
                f"'{names[0]}', e.g. '{example}' differs in {keys}. Output is "
                "stored with the first run's reference values, so outputs of "
                "these runs will be converted incorrectly.",
                stacklevel=3,
            )
