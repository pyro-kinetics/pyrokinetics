...
class GKOutputReaderCGYRO(FileReader, file_type="CGYRO", reads=GKOutput):
    ...
    def load_momentum_output(pyro):
        ...
        component_names = [
            "parallel", "perpendicular", "electromagnetic"
        ]

        # Check that the correct state exists
        if pyro.gk_code != "CGYRO":
            raise NotImplementedError(
                "load_momentum_output() can only be called for CGYRO outputs."
                )

        if pyro.gk_output is None:
            raise RuntimeError(
                "pyro.load_gk_output() must be called before load_momentum_output()."
                )

        if "momentum" not in pyro.gk_output:
            raise RuntimeError(
                "Standard momentum-flux data must be in gk_output before calling "
                "load_momentum_output()."
            )
        ...
        momentum_output = raw_data.reshape(shape, order="F")
        momentum_output = GKOutputReaderCGYRO._flux_downsample(
            momentum_output, **pyro.gk_output._downsample
        )
        momentum_output = np.swapaxes(momentum_output, 0, 2)

        # Bootstrap normalisations from standard momentum-flux data
        reference = pyro.gk_output["momentum"]

        raw_sum = momentum_output[:, :2, ...].sum(axis=1)

        scale = reference.pint.dequantify().data / raw_sum
        momentum_output = momentum_output * scale[:, None, ...]
