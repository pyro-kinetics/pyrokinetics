...
Convert the whole set to another code, then write it to a new directory. The
runs it was read from are left untouched:

.. code:: python

    cube.convert_gk_code("TGLF")
    cube.write(base_directory="tglf_runs")

This is the same as for a ``PyroScan``. ``file_name`` becomes the new code's
default (``input.TGLF``), and ``convert_gk_code`` takes a ``template_file`` for
the new input files. Code-specific settings can be applied to every run before
writing with ``add_flags``:

.. code:: python

    for pyro in cube.pyro_dict.values():
        pyro.add_flags({"nbasis_max": 6})
