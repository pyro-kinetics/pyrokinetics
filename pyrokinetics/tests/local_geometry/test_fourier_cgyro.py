...
from pyrokinetics.local_geometry.LocalGeometryFourierCGYRO import (
    grad_r,
    flux_surface,
    get_b_poloidal,
)
...
def test_flux_surface_circle():
    ...
    aR = np.array([1.0, *[0.0] * 3])
    aZ = np.array([0.0, *[0.0] * 3])

    bR = np.array([*[0.0] * 4])
    bZ = np.array([*[0.0] * 4])

    R, Z = flux_surface(theta=theta, aR=aR, aZ=aZ, bR=bR, bZ=bZ)
...
def test_flux_surface_elongation():
    ...
    lg.from_local_geometry(miller, show_fit=True)
