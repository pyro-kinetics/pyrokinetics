...
class MetricTerms(): #CleverDict
    ...
    def __init__(self, local_geometry: LocalGeometry):
        ...
        self.dZdtheta,
        self.dZdr
        ) = local_geometry.get_RZ_derivatives(self.regulartheta, normalised = True)#(local_geometry.theta)
        (
        self.d2Rdtheta2, 
        self.d2Rdrdtheta, 
        self.d2Zdtheta2, 
        self.d2Zdrdtheta
        ) = local_geometry.get_RZ_second_derivatives(self.regulartheta, normalised = True)#(local_geometry.theta)
        self.Jac = self.R * (self.dRdr * self.dZdtheta - self.dZdr * self.dRdtheta)
        self.q = local_geometry.q
        self.dpsidr = local_geometry.dpsidr
        self.Y = integrate.trapezoid(self.Jac / (self.R ** 2), self.regulartheta) / (2.0 * np.pi)
    ...
    def get_toroidal_system_covariant_metric_derivatives(self):
        ...
        self._partial_g_vrvtheta_partial_vtheta = self.d2Rdrdtheta * self.dRdtheta + self.d2Rdtheta2 * self.dRdr + self.d2Zdrdtheta * self.dZdtheta + self.d2Zdtheta2 * self.dZdr
        self._partial_g_vthetavtheta_partial_vr = 2 * (self.dRdtheta * self.d2Rdrdtheta + self.dZdtheta * self.d2Zdrdtheta)
        self._partial_Jac_partial_vtheta = (self.dRdtheta * self.Jac / self.R) + self.R * (self.d2Rdrdtheta * self.dZdtheta + self.dRdr * self.d2Zdtheta2 - self.d2Rdtheta2 * self.dZdr - self.dRdtheta * self.d2Zdrdtheta)
        check = integrate.cumulative_trapezoid(self._partial_Jac_partial_vtheta, self.regulartheta)
        check = list(check)
        check.insert(0,0.0)
        check = np.array(check)
        self._check = check
        return self._partial_g_vrvtheta_partial_vtheta, self._partial_g_vthetavtheta_partial_vr, self._partial_Jac_partial_vtheta, self._check #self._partial_g_vrvr_partial_vtheta, self._check
    ...
    def get_dB_cov_zeta_dr(self):
        g_vrvr, g_vrvtheta, g_vthetavtheta, g_zetazeta = self.get_toroidal_system_covariant_metric_comps
        partial_g_vrvtheta_partial_vtheta, partial_g_vthetavtheta_partial_vr, partial_Jac_partial_vtheta, check = self.get_toroidal_system_covariant_metric_derivatives
        B_cov_zeta = self.get_B_cov_zeta
        H = self.Y + ((self.q / self.Y) ** 2) * (integrate.trapezoid(self.Jac ** 3 / ((self.R ** 4) * g_vthetavtheta), self.regulartheta) / (2.0 * np.pi))
        term1 = self.Y * self.dqdr / self.q
        term2 = - (integrate.trapezoid(-2.0 * self.Jac * self.dRdr / (self.R ** 3), self.regulartheta) / (2.0 * np.pi))
        term3 = - (self.mu_0 * self.dPdr / (self.dpsidr ** 2)) * (integrate.trapezoid(self.Jac ** 3 / ((self.R ** 2) * g_vthetavtheta), self.regulartheta) / (2.0 * np.pi))
        to_integrate = (self.Jac / ((self.R ** 2) * g_vthetavtheta)) * (partial_g_vrvtheta_partial_vtheta - partial_g_vthetavtheta_partial_vr - (g_vrvtheta * partial_Jac_partial_vtheta / self.Jac))
    ...
    def get_dJac_dr(self):
        ...
        term2 = - (self.Jac / g_vthetavtheta) * (partial_g_vrvtheta_partial_vtheta - partial_g_vthetavtheta_partial_vr - (g_vrvtheta * partial_Jac_partial_vtheta / self.Jac))
        term3 = (self.mu_0 * self.dPdr / (self.dpsidr ** 2)) * (self.Jac ** 3) / g_vthetavtheta
        term4 = (B_cov_zeta * dB_cov_zeta_dr / (self.dpsidr ** 2)) * (self.Jac ** 3) / ((self.R ** 2) * g_vthetavtheta)
    ...
    def get_d2alpha_drdvtheta(self): # sometimes known as 'local shear'
        ...
        term2 = - self.d2psidr2 * self.Jac * B_cov_zeta / ((self.dpsidr ** 2) * (self.R ** 2))
        term3 = B_cov_zeta * dJac_dr / (self.dpsidr * (self.R ** 2))
        term4 = - (2.0 * self.dRdr / (self.R ** 3)) * (B_cov_zeta * self.Jac / self.dpsidr)
    ...
    def get_dalpha_dr(self):
        ...
        self._dalpha_dr = dalpha_dr - f(0.0) # set dalpha/dr(r,theta=0.0)=0.0, assumed by codes
    ...
    def get_FA_covariant_metric_components(self):
        ...
        return self._g_rr, self._g_ralpha, self._g_rt, self._g_alphaalpha, self._g_alphat, self._g_tt
