...
fields.isel(nmode=0).sel(field="phi").plot(marker="x", label="mode 1")
fields.isel(nmode=1).sel(field="phi").plot(marker="o", label="mode 2")
...
plt.subplot(2, 3, 2)
fields = pyro.gk_output.data["fields"]
fields.isel(nmode=0).sel(field="apar").plot(marker="x", label="mode 1")
fields.isel(nmode=1).sel(field="apar").plot(marker="o", label="mode 2")
plt.show(block=False)
plt.legend()
plt.title("Apar")
