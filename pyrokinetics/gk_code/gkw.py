...
class GKInputGKW(GKInput):
    ...
    code_name = "GKW"
    default_file_name = "input.gkw"
    norm_convention = "gs2"#TODO
    ...
    pyro_gkw_miller_turnbull_defaults = {
        "rho": 0.16666,
        "q": 2.0,
        "kappa": 1.0,
        "s_kappa": 0.0,
    ...
    def write(self, filename: PathLike, float_format: str = "", local_norm=None):
        ...
        for name, namelist in self.data.items():
            self.data[name] = convert_dict(namelist, local_norm.gs2)#TODO
    ...
    def get_local_geometry(self) -> LocalGeometry:
        ...
        if geometry_type == "miller":
            if (
                self.data["geom"].get("square",0.0) != 0.0
                or self.data["geom"].get("ssquare",0.0) != 0.0
            ):
    ...
    def get_local_geometry_miller(self) -> LocalGeometryMiller:
        ...
        for (pyro_key, (gkw_param, gkw_key)), gkw_default in zip(
            self.pyro_gkw_miller.items(),
            self.pyro_gkw_miller_defaults.values()
        ):
            miller_data[pyro_key] = self.data[gkw_param].get(gkw_key, gkw_default)

        minor_a = self.data["pyrokinetics_info"]["minor_r"]
        major_R = self.data["pyrokinetics_info"]["major_r"]
        miller_data["Rmaj"] = major_R / minor_a
        ...
        # TODO this may be determined in GKW internally using "sp"
        #       - introduce check
        #           - then: calculate from kinetic species
    ...
    def get_local_geometry_miller_turnbull(self) -> LocalGeometryMillerTurnbull:
        ...
        for (pyro_key, (gkw_param, gkw_key)), gkw_default in zip(
            self.pyro_gkw_miller_turnbull.items(),
            self.pyro_gkw_miller_turnbull_defaults.values()
        ):
            miller_data[pyro_key] = self.data[gkw_param].get(gkw_key, gkw_default)
        ...
        # TODO this may be determined in GKW internally using "sp"
        #       - introduce check
        #       - action - must be calculated from kineic species
        miller_data["beta_prime"] = self.data["spcgeneral"]["betaprime_ref"] / miller_data["Rmaj"]
        # must construct using from_gk_data as we cannot determine bunit_over_b0 here
        return LocalGeometryMillerTurnbull.from_gk_data(miller_data)
    ...
    def get_numerics(self) -> Numerics:
        ...
        # Set time stepping
        dtim = self.data["control"].get("dtim", 0.005)
        naverage = self.data["control"].get("naverage", 100)
        ntime = self.data["control"].get("ntime", 100) 
        numerics_data["delta_time"] = dtim / sqrt2  # TODO multiply with Rmaj
        numerics_data["max_time"] = dtim * naverage * ntime
        ...
        numerics_data["kx"] = self.data["mode"].get("chin",0)   #TODO this is ~kr (krnorm)
